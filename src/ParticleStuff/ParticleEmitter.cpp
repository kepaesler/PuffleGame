...
void ParticleEmitter::draw() {
	if (visible) {
		...
		case DirectionalEmitter:
			ofDrawSphere(position.x, position.y, radius/10);  // just draw a small sphere for point emitters 
			break;
		case SphereEmitter:
		case RadialEmitter:
			ofDrawSphere(position.x , position.y, radius/10);  // just draw a small sphere as a placeholder
			break;
		default:
			break;
		}
